    ...
    server.route([
      {
        ...
        handler: function (req, reply) {
          reply(self.wsdl.toXML()).type('text/xml');
        }
      },
      {
        method: 'POST',
        ...
        handler: function (req, reply) {
          try {
            ...
            self._processRequestXml(req, reply, req.payload);
          }
          catch (err) {
...
HapiServer.prototype._processRequestXml = function (req, reply, xml) {
  var self = this;
  try {
    self.logMsg("received", xml);

    self._process(xml, req, function (result, statusCode, responseHeaders) {
...
HapiServer.prototype._executeMethod = function (options, req, callback, includeTimestamp) {
  ...
  function handleResult(error, result) {
    ...
    callback(self._envelope(body, includeTimestamp), 200, responseHeaders);
  }
  ...
  method(args, handleResult, options.headers, req);
};
